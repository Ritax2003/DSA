...
int replaceWithLargerNodesSumhelp(BinaryTreeNode<int> *root,int sum) {
    if(root==NULL){
        return sum;
    }
    sum = replaceWithLargerNodesSumhelp(root->right,sum);
    sum +=root->data;
    root->data=sum;
    sum = replaceWithLargerNodesSumhelp(root->left,sum);
    return sum;
}
void replaceWithLargerNodesSum(BinaryTreeNode<int> *root){
    int sum =0;
    replaceWithLargerNodesSumhelp(root,sum);
}
int main() {
